...
summon minecraft:marker ~ ~ ~ {data:{x:0, y:0, z:0},Tags:["mcs_home_3"]}

# Store Data:
execute as @e[type=minecraft:marker,distance=0..5,sort=nearest,tag=mcs_home_3] at @s store result entity @s data.x int 1 run scoreboard players get @p home3_x
execute as @e[type=minecraft:marker,distance=0..5,sort=nearest,tag=mcs_home_3] at @s store result entity @s data.y int 1 run scoreboard players get @p home3_y
