...
# Teleport Player
execute if score teleport perms_cooldown matches 0 run function mcs_member:commands/homes/tp/data/2 with entity @e[type=minecraft:marker,distance=0..5,sort=nearest,tag=mcs_home_2,limit=1] data
execute if score teleport perms_cooldown matches 20.. run tag @s add tp_cooldown_home_2

# Teleport Msg:
tellraw @s [{"text": "Teleporting to ", "color": "#a3a3a3"}, {"text": "home 2", "color": "red"}, {"text": "...", "color": "#a3a3a3"}]

# Remove Entity:
execute if score teleport perms_cooldown matches 0 as @e[type=minecraft:marker,tag=mcs_home_2] at @s run kill @s
